...
<main class="app-content">
   <div class="app-title">
      <div>
         <h1><i class="fa fa-dashboard"></i> Organizations</h1>
      ...
      <ul class="app-breadcrumb breadcrumb">
         ...
         <li class="breadcrumb-item"><a href="<?php echo base_url().'admin/organization/index';?>">Organizations</a></li>
            ...
            <div class="tile-body">
               <form name="sortForm" id="sortForm" method="get" class="row" action="" autocomplete="off">
                  <div class="form-group col-md-8">
                     <div class="row">
                        <div class="form-group col-md-8">
                           <input class="form-control" name="search_kw" id="search_kw" type="text" placeholder="Search Organizations..." value="<?php echo $search_kw; ?>">
                        </div>
               ...
               <?php
                  if(!empty($organization_data['organizations']))
                  {
                    $current_url = explode('?msg', $_SERVER['REQUEST_URI']);
                   
                    $current_url = $current_url[0];
                    $url = base_url().'admin/organization/index';
                  ?>
               <?php echo form_open_multipart($current_url, array('name' => 'organization_form', 'id' => 'organizationsForm', 'autocomplete'=>'off')); ?>
                  <div class="row">
                     <div class="col-md-12">
                        <?php echo $organization_data['total_items']; ?></strong> records</span>
                     </div>
                  </div>
                  <div class="table-responsive">
                     <table class="table table-bordered table-hover">
                        ...
                        <tbody>
                           <?php foreach($organization_data['organizations'] as $organization){ ?>
                           <tr>
                              <td align="left" valign="top"> <?php echo $organization->orgn_id; ?> </td>
                              <td align="left" valign="top"> <?php echo $organization->orgn_name; ?></td>
                              <td align="left" valign="top">
                                 <input class="<?php echo array_key_exists($organization->orgn_id, $existing_alias) ? "error" : ""; ?>" value="<?php echo (!$data_posted || array_key_exists($organization->orgn_id, $existing_alias)) ? $organization->alias : set_value('alias['.$organization->orgn_id.']'); ?>" name="alias[<?php echo $organization->orgn_id; ?>]" type="text" style="width:100%" value="<?php echo $organization->alias; ?>">
                                 <?php if(array_key_exists($organization->orgn_id, $existing_alias)){ ?>
                                    <span class="error"><?php echo $existing_alias[$organization->orgn_id]; ?></span>
                                 <?php } ?>
                              </td>
                              <td align="left" valign="top">
                                 <button class="btn btn-sm btn-info btn-block updateAlias" type="button"  data-id="<?php echo $organization->orgn_id; ?>">Update</button>
                              </td>
                           </tr>
                           <?php } ?>
                        </tbody>
...
<script>
   jQuery(document).ready(function($){
       ...
       $("#organizationsForm").on('submit',function (e) {
       ...
       $('#organizationsForm input[type="text"]').on('checkAlias keyup',function() {
       ...
       $('.updateAlias').click(function(event){
         ...
         var orgnId = updateBtn.attr('data-id');
         ...
         $.ajax({
                url: "<?php echo base_url().'/admin/organization/update_alias'; ?>",
                type: 'GET',
                dataType: 'json',
                data: { alias: alias, orgn_id : orgnId},
