...
<main class="app-content">
    <div class="app-title">
        <div>
            <h1><i class="fa fa-dashboard"></i> States</h1>
        ...
        <ul class="app-breadcrumb breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="<?php echo base_url().'admin/state/index';?>">States</a></li>
            ...
            <div class="tile">
                <div class="tile-body">
                  ...
                  <div class="form-group col-md-8">
                     <div class="row">
                        <div class="form-group col-md-8">
                           <input class="form-control" name="search_kw" id="search_kw" type="text" placeholder="Search states..." value="<?php /*echo $search_kw; */?>">
                        </div>
                  ...
                  <div class="form-group col-md-4">
                    ...
                    if(!empty($state_data['states']))
                    {
                        ...
                        ?>
                        <?php echo form_open_multipart($current_url, array('name' => 'states_form', 'id' => 'statesForm', 'autocomplete'=>'off')); ?>
                        <div class="row">
                            <div class="col-md-12">
                                <?php echo $state_data['total_items']; ?></strong> records</span>
                            </div>
                        </div>
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover">
                                ...
                                <tbody>
                                <?php foreach($state_data['states'] as $state){?>
                                    <tr>
                                        <td align="left" valign="top"> <?php echo $state->state_id; ?> </td>
                                        <td align="left" valign="top"> <?php echo $state->state_name; ?></td>
                                        <td align="left" valign="top">
                                            <input class="<?php echo array_key_exists($state->state_id, $existing_alias) ? "error" : ""; ?>" value="<?php echo (!$data_posted || array_key_exists($state->state_id, $existing_alias)) ? $state->alias : set_value('alias['.$state->state_id.']'); ?>" name="alias[<?php echo $state->state_id; ?>]" type="text" style="width:100%" value="<?php echo $state->alias; ?>">
                                            <?php if(array_key_exists($state->state_id, $existing_alias)){ ?>
                                                <span class="error"><?php echo $existing_alias[$state->state_id]; ?></span>
                                            <?php } ?>
                                        </td>
                                        <td align="left" valign="top">
                                            <button class="btn btn-sm btn-info btn-block updateAlias" type="button"  data-id="<?php echo $state->state_id; ?>">Update</button>
                                        </td>
                                    </tr>
                                <?php } ?>
                                </tbody>
...
<script>
    jQuery(document).ready(function($){
        ...
        $("#statesForm").on('submit',function (e) {
        ...
        $('#statesForm input[type="text"]').on('checkAlias keyup',function() {
        ...
        $('.updateAlias').click(function(event){
            ...
            var stateId = updateBtn.attr('data-id');
            ...
            $.ajax({
                url: "<?php echo base_url().'/admin/state/update_alias'; ?>",
                type: 'GET',
                dataType: 'json',
                data: { alias: alias, state_id : stateId},
